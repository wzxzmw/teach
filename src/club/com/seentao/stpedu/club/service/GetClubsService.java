...
public class GetClubsService {
	 ...
	 */
	@Transactional
	public String getClubsInfo(String userId, String memberId, String searchWord,
		...
		if(inquireType == GameConstants.GET_CLUB_INFORMATION ){
				...
				for (ClubMain clubMain : appQueryPage.getList()) {
					...
					if(centerAttention.size()>0){
						...
						for (CenterAttention centerAttention2 : centerAttention) {
							if(Integer.valueOf(centerAttention2.getRelObjetId()).equals(clubMain.getClubId())){
								a= true;
								break;
							}
						}
		...
		}else if(inquireType == GameConstants.GET_THE_CLUB_INFORMATION){
			//获取赛事俱乐部信息
			//查询参赛俱乐部表获得俱乐部id
			/*List<ArenaJoinClub> list = arenaJoinClubDao.selectAllArenaJoinClub();
			if(null == list || list.size()<=0){
				LogUtil.error(this.getClass(), "getClubsInfo", "赛事信息为空");
				return Common.getReturn(AppErrorCode.ERROR_TYPE_FIVE, AppErrorCode.CLUB_INFORMATION_IS_EMPTY).toJSONString();
			}*/
			try {
				// 根据当前赛事id查询赛事下的俱乐部
				List<Integer> ls = arenaJoinClubDao.queryArenaJoinClubByCompId(Integer.valueOf(gameEventId));
				//根据俱乐部id查询参赛俱乐部获取俱乐部参赛信息
				Map<String ,Object> paramMap = new HashMap<String ,Object>();
				paramMap.put("clubId", StringUtil.ListToString(ls, ","));
				//调用分页组件
				QueryPage<ClubMain> appQueryPage = QueryPageComponent.queryPageExecute(limit, start, paramMap, ClubMain.class,"queryCountclubmatch","queryByclubmainmatch");
				//msg错误码
				if(!appQueryPage.getState()){
					appQueryPage.error(AppErrorCode.ERROR_CLUBMAIN);
				}
				for (ClubMain clubMain : appQueryPage.getList()) {
					 ...
					 */
					clubMain.setClubLogoLink(Common.checkPic(pic.getFilePath()) == true ? pic.getFilePath()+ActiveUrl.HEAD_MAP:pic.getFilePath());
					//clubMain.setClubLogoLink(pic==null ?"":pic.getFilePath()+ActiveUrl.HEAD_MAP);
				}
				return appQueryPage.getMessageJSONObject("clubs").toJSONString();
			} catch (Exception e) {
		...
		}else if(inquireType == GameConstants.GET_ATTENTION_CLUB_INFORMATION){
		...
		}else if(inquireType == GameConstants.GET_NEW_CLUB_INFORMATION){
			//查询最热俱乐部信息，根据人员数量
			Map<String ,Object> paramMap = new HashMap<String ,Object>();
			//调用分页组件
			...
			return appQueryPage.getMessageJSONObject("clubs").toJSONString();
		}else if(inquireType == GameConstants.GET_HOT_CLUB_INFORMATION){
			//查询最新俱乐部
			Map<String ,Object> paramMap = new HashMap<String ,Object>();
			//调用分页组件
			...
			return appQueryPage.getMessageJSONObject("clubs").toJSONString();
		}else if(inquireType == GameConstants.GET_MY_CLUB_INFORMATION){
			...
			try {
				...
				if(picture == null){
					...
					json.put("gameBannerLink", picture.getFilePath()+ActiveUrl.ROTATION_MAP);
				}
				//培训海报id
				...
				if(ps == null){
					 ...
					 * 压缩图片
					 */
					json.put("teachingBannerLink", Common.checkPic(ps.getFilePath()) == true ?ps.getFilePath()+ActiveUrl.ROTATION_MAP:ps.getFilePath());
				}
				//风格海报id
				...
				if(pr == null){
					...
					json.put("styleBannerLink", Common.checkPic(pr.getFilePath()) == true ? pr.getFilePath()+ActiveUrl.ROTATION_MAP:pr.getFilePath());
				}
				...
				if(school == null){
					...
					json.put("schoolId",String.valueOf(school.getSchoolId()));
				}
				//查询俱乐部所在地区
		...
		}else if(inquireType == GameConstants.GET_MY_CLUB_INFORMATION_ACTIVE_DEGREE){
			...
			try {
				...
				if(picture == null){
					 ...
					 * 压缩图片
					 */
					json.put("gameBannerLink", Common.checkPic(picture.getFilePath()) == true ? picture.getFilePath()+ActiveUrl.ROTATION_MAP:picture.getFilePath());
				}
				//培训海报id
				json.put("teachBannerId", clubmaim.getTeachBannerId());
				String te = RedisComponent.findRedisObjectPublicPicture(clubmaim.getTeachBannerId(),0);
